...
from download_data import french_TDE_names, french_TDE_coords
from paper_data import french_TDE_redshifts
...
def fit_SED(objID, bands="griz", redshift=0, magnitudes_dict=None, extension=""):
    ...
    with open(f"prospector/data/{french_TDE_names[objID]}_LEGACY_mfrac.txt", "w") as text_file:
    ...
    hfile = f"prospector/fits_data/{french_TDE_names[objID]}_prospector_{extension}.h5"
    ...
    objID = objID + 1
    fit_SED(objID, bands="griz", redshift=french_TDE_redshifts[objID], magnitudes_dict=french_TDE_magnitudes_dicts[objID], extension=extension)
...
def getStellarMass(objID, extension="SED"):
    if False:
        ...
        # Taking the fitted total formed mass and then multiplying it by the surviving fraction approximation
        hfile = f"prospector/fits_data/{french_TDE_names[objID]}_prospector_{extension}.h5"
        mfrac = np.float64(open(f"prospector/data/{french_TDE_names[objID]}_LEGACY_mfrac.txt","r").read())
        out, out_obs, out_model = reader.results_from(hfile)
        mass = out["chain"][:,0]
        mass = mass*mfrac
...
if __name__ == "__main__":
    ...
    french_TDE_magnitudes_dicts = []
    for i in range(len(french_TDE_names)):
        french_TDE_magnitudes_dicts.append({"name":french_TDE_names[i]})
        for band in "griz":
            try:
                french_TDE_magnitudes_dicts[i][f"cModelMag_{band}"] = french_TDE_unreddenedMagnitudes[i][band]
                french_TDE_magnitudes_dicts[i][f"cModelMagErr_{band}"] = 0
            except:
                pass

    for i in range(len(french_TDE_magnitudes_dicts)):
        french_TDE_magnitudes_dicts[i] = makeAstropyTableFromDictionnary(french_TDE_magnitudes_dicts[i])
    ...
    if input("Fit French TDEs? [y/n]") == "y":
        ...
        fit_SED(objID, bands="griz", redshift=french_TDE_redshifts[objID], magnitudes_dict=french_TDE_magnitudes_dicts[objID], extension="FINAL")
