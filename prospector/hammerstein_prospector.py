...
from download_data import hammerstein_TDE_names, remaining_hammerstein_TDE_names, hammerstein_TDE_coords
from paper_data import remaining_hammerstein_TDE_redshifts
...
def fit_SED(objID, bands="griz", redshift=0, magnitudes_dict=None, extension=""):
    ...
    with open(f"prospector/data/{remaining_hammerstein_TDE_names[objID]}_LEGACY_mfrac.txt", "w") as text_file:
    ...
    hfile = f"prospector/fits_data/{remaining_hammerstein_TDE_names[objID]}_prospector_{extension}.h5"
    ...
    rewrite_mfracFiles(objID, extension=extension)
    objID = objID + 1
    fit_SED(objID, bands="griz", redshift=remaining_hammerstein_TDE_redshifts[objID], magnitudes_dict=hammerstein_TDE_magnitudes_dicts[objID], extension=extension)


def rewrite_mfracFiles(objID, extension=""):
    hfile = f"prospector/fits_data/{remaining_hammerstein_TDE_names[objID]}_prospector_{extension}.h5"
...
def getStellarMass(objID, extension="SED"):
    if False:
        # Directly fitting the surviving mass
        hfile = f"prospector/fits_data/{objects_names[objID]}_survMass_prospector_SED.h5"
        out, out_obs, out_model = reader.results_from(hfile)
        ...
        # Taking the fitted total formed mass and then multiplying it by the surviving fraction approximation
        hfile = f"prospector/fits_data/{remaining_hammerstein_TDE_names[objID]}_prospector_{extension}.h5"
        mfrac = np.float64(open(f"prospector/data/{remaining_hammerstein_TDE_names[objID]}_LEGACY_mfrac.txt","r").read())
        out, out_obs, out_model = reader.results_from(hfile)
        mass = out["chain"][:,0]
        mass = mass*mfrac
...
with open('hammerstein_TDE_magnitudes.pkl', 'rb') as f:
    hammerstein_TDE_unreddenedMagnitudes = pickle.load(f)


# Do the magnitudes thing
if __name__ == "__main__":

    hammerstein_TDE_magnitudes_dicts = []
    for i in range(len(remaining_hammerstein_TDE_names)):
        hammerstein_TDE_magnitudes_dicts.append({"name":remaining_hammerstein_TDE_names[i]})
        for band in "griz":
            try:
                hammerstein_TDE_magnitudes_dicts[i][f"cModelMag_{band}"] = hammerstein_TDE_unreddenedMagnitudes[i][band]
                hammerstein_TDE_magnitudes_dicts[i][f"cModelMagErr_{band}"] = 0
            except:
                pass

    for i in range(len(hammerstein_TDE_magnitudes_dicts)):
        hammerstein_TDE_magnitudes_dicts[i] = makeAstropyTableFromDictionnary(hammerstein_TDE_magnitudes_dicts[i])


    print(hammerstein_TDE_magnitudes_dicts)
