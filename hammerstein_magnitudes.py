...
hammerstein_TDE_magnitudes = []
hammerstein_TDE_unreddenedMagnitudes = []
for i in range(len(hammerstein_TDE_names)):
    if hammerstein_TDE_names[i] in remaining_hammerstein_TDE_names:
        hammerstein_TDE_magnitudes.append({})
        hammerstein_TDE_unreddenedMagnitudes.append({})
        for band in "griz":
            try:
                _, _, mag = get_n_and_r50(i, "Bulge", redshift=hammerstein_TDE_redshifts[i], band=band, survey="DESI_PSF_FINAL2")
                hammerstein_TDE_magnitudes[-1][band] = mag
                hammerstein_TDE_unreddenedMagnitudes[-1][band] = mag - hammerstein_TDE_extinction[hammerstein_TDE_names[i]][band]
            except:
                pass


hammerstein_TDE_unreddenedMagnitudes = np.array(hammerstein_TDE_unreddenedMagnitudes)


if __name__ == "__main__":

    print(hammerstein_TDE_magnitudes)
    printPropertyAcrossFilters(hammerstein_TDE_magnitudes, "Magnitude")
    printPropertyAcrossFilters(hammerstein_TDE_unreddenedMagnitudes, "Dereddened magnitude")

    if input("Overwrite the pickle files used in prospector_myFits? [y/n]") == "y":
        with open('hammerstein_TDE_magnitudes.pkl', 'wb') as f:
            pickle.dump(hammerstein_TDE_unreddenedMagnitudes, f)