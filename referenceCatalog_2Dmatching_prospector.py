...
QPE_galaxies = np.loadtxt("QPE_distribution.txt")
QPE_galaxies[:,3] = QPE_galaxies[:,3]+0.2370666
np.savetxt("QPE_distribution_offset.txt", QPE_galaxies)
fieldnames = ["bt_ratio", "n_sersic", "smsd", "m_star", "m_bh", "redshift"]
QPEGalaxies = pd.read_csv("QPE_distribution_offset.txt", header=None, names=fieldnames, delimiter=" ")
...
np.savetxt("referenceCatalog_final_prospector.txt", goodGalaxies)
refCat = np.loadtxt("referenceCatalog_final_prospector.txt")
fieldnames = [f"col_{i}" for i in range(refCat.shape[1])]
fieldnames[1] = "redshift"
fieldnames[63] = "m_star"
fieldnames[67] = "m_bh"
refCat = pd.read_csv("referenceCatalog_final_prospector.txt", delimiter=" ", header=None, names=fieldnames)
