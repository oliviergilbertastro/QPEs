...
This uses the reference catalog WITH black hole mass uncertainties.
"""
import pickle
import numpy as np
...
if __name__ == "__main__":
    ...
    m_star_offsets = [0,0]
    referenceCata_extension = "_9bins"


    # load reference catalog
    refCat = np.loadtxt(f"referenceCatalog_with_uncertainties_final{referenceCata_extension}.txt")
    fieldnames = [f"col_{i}" for i in range(refCat.shape[1])]
    refCat = pd.read_csv(f"referenceCatalog_with_uncertainties_final{referenceCata_extension}.txt", delimiter=" ", header=None, names=fieldnames)
    ...
    redshiftMass([QPE_data, TDE_data], referenceCatalogData=refCat, columns_compare=(4,102,100), save_plot="french_redshift_distribution", fontsize=16, markersize=10,
    ...
    myCombinedFinalPlot([QPE_data, TDE_data], referenceCatalogData=refCat, columns_compare=((96,15,105),102,100), save_plot="french_combined_final", fontsize=16, markersize=10,
                        levels=[0.5,0.7,0.9,1],
                        smoothness=12,
    ...
    myCornerPlot(
        ...
        refCat=refCat,
        bw_adjust=3,
        columns_compare=[100,102,4,77,15,96,105],
        save_plot="french_corner_plot_more",
    ...
    myCornerPlot(
        ...
        refCat=refCat,
        bw_adjust=3,
        columns_compare=[100,102,15,96,105],
