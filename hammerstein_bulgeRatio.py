...
band = "g"
hammerstein_TDE_bulgeRatios = []
for i in range(len(remaining_hammerstein_TDE_names)):
    fluxes, labels = get_fluxes(f"{remaining_hammerstein_TDE_names[i]}_{band}-band_{'Bulge'}_{survey}.pkl")
    ...
    if bulge_ratio[0] != bulge_ratio[0]:
        bulge_ratio = (0,0,0)
    hammerstein_TDE_bulgeRatios.append(bulge_ratio)
hammerstein_TDE_bulgeRatios = np.array(hammerstein_TDE_bulgeRatios)

if __name__ == "__main__":
    plt.errorbar(range(len(remaining_hammerstein_TDE_names)), hammerstein_TDE_bulgeRatios[:,0], [hammerstein_TDE_bulgeRatios[:,1], hammerstein_TDE_bulgeRatios[:,2]], fmt="o", label="Ham. TDEs")
    plt.legend()
    plt.ylabel(r"$(B/T)_g$", fontsize=16)
    plt.xlabel("Index", fontsize=16)
    plt.show()
    if input("Overwrite the text files used in legacy_vs_legacy? [y/n]") == "y":
        np.savetxt("hammerstein_TDE_bulgeRatios.txt", hammerstein_TDE_bulgeRatios)