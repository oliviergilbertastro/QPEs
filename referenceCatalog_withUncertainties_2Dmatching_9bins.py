...
refCat = cut_from_catalog(refCat, 4, bounds["redshift"], verbose=True)
refCat = cut_from_catalog(refCat, (102 if matching == "m_star" else 100), bounds[matching], verbose=True)
np.savetxt("referenceCatalog_with_uncertainties_matching.txt", refCat)
fieldnames = [f"col_{i}" for i in range(refCat.shape[1])]
fieldnames[4] = "redshift"
fieldnames[102] = "m_star"
fieldnames[100] = "m_bh"
refCat = pd.read_csv("referenceCatalog_with_uncertainties_matching.txt", delimiter=" ", header=None, names=fieldnames)
...
refCat = np.loadtxt("referenceCatalog_with_uncertainties_matching.txt")
