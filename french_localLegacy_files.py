...
if __name__ == "__main__":
    ...
    french_TDE_mBH = add_0_uncertainties(np.array(french_TDE_mBH))

    # Make data distributions
    for i in [french_TDE_bulgeRatios, french_TDE_sersicIndices, french_TDE_SMSDs, french_TDE_stellar_masses, french_TDE_mBH, french_TDE_r50s]:
        print(i.shape)
    french_TDE_data = np.array([french_TDE_bulgeRatios, french_TDE_sersicIndices, french_TDE_SMSDs, french_TDE_stellar_masses, french_TDE_mBH, french_TDE_r50s]).T
    print(french_TDE_data.shape)

    # Need to save it in 3 separate files because "no 3D arrays" :(
    for i in range(3):
        np.savetxt(f"french_TDE_allRelevantData_{i}.txt", french_TDE_data[i,:,:])